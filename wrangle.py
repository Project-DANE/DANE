...
def col_drop(df):
    df = df.drop(columns = ['id', 'total_charges', 'amount_paid', 'open', 'charge_code', 'description', 'charge_name', 
                            'sStatus', 'reason', 'GuarantorRequired'])
    return df
...
def model_prep(df):
    ...
    return train, y_train, y_val, y_test, train_scaled, val_scaled, test_scaled
